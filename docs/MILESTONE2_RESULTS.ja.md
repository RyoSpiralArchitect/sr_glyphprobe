...
Milestone 2はC1 v1をmodel入力やoutcome解析に使っていない。ただし、別に記録した研究文脈への露出により、現在はbank全体を廃止している。詳細は[holdout状態](HOLDOUT_STATUS.ja.md)を参照する。未使用bankとして表現したり、再利用したりしない。
...
運用上のMilestone 2は完了した。Layer 2は、新しい対象限定型の因果プロトコルを設計できる段階に入った。Layer 4は未解決のままで、候補にはしない。C1 v1はこの実験に使えない。露出した研究文脈の外で新しいversionのbankを用意し、候補、介入部位と操作、endpoint、対照、多重性familyを公開凍結するまで未使用に保つ。この判断で認めるのはプロトコル設計であり、因果主張ではない。
