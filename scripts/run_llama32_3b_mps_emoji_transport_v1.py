#!/usr/bin/env python3
"""Run the frozen Llama 3.2 3B MPS emoji-transport grid sequentially.

The launcher reads no scientific output. It verifies the frozen preflight,
starts ten independent GlyphProbe processes in the prespecified order, stops on
the first non-zero exit, and writes one atomic execution receipt only after all
ten processes complete. Separate processes guarantee that each full model is
released before the next model is loaded.
"""
...
LAUNCHER_LOG_RELATIVE = Path("runs/.e2-llama32-3b-mps-transport-v1-launcher-logs")
CONFIG_ORDER = (
    "configs/e2_llama32_3b_mps_full50_sky_v1.yaml",
    "configs/e2_llama32_3b_mps_full50_food_v1.yaml",
    "configs/e2_llama32_3b_mps_full50_animals_v1.yaml",
    "configs/e2_llama32_3b_mps_full50_transport_v1.yaml",
    "configs/e2_llama32_3b_mps_full50_social_v1.yaml",
    "configs/e2_llama32_3b_mps_core35_sky_v1.yaml",
    "configs/e2_llama32_3b_mps_core35_food_v1.yaml",
    "configs/e2_llama32_3b_mps_core35_animals_v1.yaml",
    "configs/e2_llama32_3b_mps_core35_transport_v1.yaml",
    "configs/e2_llama32_3b_mps_core35_social_v1.yaml",
)
RUN_NAMES = (
    "e2-llama32-3b-mps-full50-sky-transport-v1",
    "e2-llama32-3b-mps-full50-food-transport-v1",
    "e2-llama32-3b-mps-full50-animals-transport-v1",
    "e2-llama32-3b-mps-full50-transport-transport-v1",
    "e2-llama32-3b-mps-full50-social-transport-v1",
    "e2-llama32-3b-mps-core35-sky-transport-v1",
    "e2-llama32-3b-mps-core35-food-transport-v1",
    "e2-llama32-3b-mps-core35-animals-transport-v1",
    "e2-llama32-3b-mps-core35-transport-transport-v1",
    "e2-llama32-3b-mps-core35-social-transport-v1",
)
...
def _validate_preflight(root: Path, preflight_path: Path) -> dict[str, Any]:
    ...
    authorization = _require_mapping(
        preflight.get("authorization"), "authorization binding"
    )
...
def _validate_git_freeze(root: Path, preflight: dict[str, Any]) -> dict[str, str]:
    ...
    changed = {
        ...
        for line in _git(
            ...
            "diff",
            "--name-only",
            f"{audited_commit}..{head}",
        ).splitlines()
        if line.strip()
    }
    if changed != {PREFLIGHT_RELATIVE.as_posix()}:
        raise ExecutionError(
            "Only the frozen preflight receipt may change after audited_commit"
        )
    return {
    ...
    *,
    ...
    try:
        for index, relative in enumerate(CONFIG_ORDER):
            ...
            cell_finished = datetime.now(timezone.utc).isoformat()
            record = {
                **active_cell,
            ...
            if result.returncode != 0:
                ...
                raise ExecutionError(
                    f"Frozen run failed at {relative} with return code "
                    f"{result.returncode}"
                )
        ...
        raise ExecutionError("Frozen run interrupted; v1 may not resume") from exc
        ...
        if isinstance(exc, ExecutionError):
            raise
        raise ExecutionError("Frozen launcher failed; v1 may not resume") from exc
